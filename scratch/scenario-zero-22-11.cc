...
//
void ChangeInterval(Ptr<Application> app, Ptr<UniformRandomVariable> rand) {
    double newInterval = rand->GetValue(100.0, 1000.0); // Random between 100us and 1000us
...
//
// Add constant interference to Group 2
// Increase noise figure for Group 2
//
//
...
Ptr<UniformRandomVariable> intervalRng = CreateObject<UniformRandomVariable>();
intervalRng->SetAttribute("Min", DoubleValue(100.0));
intervalRng->SetAttribute("Max", DoubleValue(1000.0));

for (uint32_t i = 0; i < clientApp.GetN(); ++i) {
    Ptr<Application> app = clientApp.Get(i);
    app->SetAttribute("Interval", TimeValue(MicroSeconds(intervalRng->GetValue())));
    Simulator::Schedule(Seconds(1.0), &ChangeInterval, app, intervalRng);
}
  ...
  //
  //
 // Debug: Print number of devices on each node
  for (uint32_t i = 0; i < ueNodes.GetN(); ++i) {
      Ptr<Node> node = ueNodes.Get(i);
      NS_LOG_UNCOND("Node " << node->GetId() << " has " << node->GetNDevices() << " devices.");
  }
  //
  //
  if (enableTraces)
