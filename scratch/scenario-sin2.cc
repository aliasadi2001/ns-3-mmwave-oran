...
main (int argc, char *argv[])
{
 ...
 // Ptr<MmWavePointToPointEpcHelper> epcHelper = CreateObject<MmWavePointToPointEpcHelper> ();
  ...
  Ptr<MmWaveHelper> mmwaveHelperGroup1 = CreateObject<MmWaveHelper>();
  mmwaveHelperGroup1->SetPathlossModelType("ns3::ThreeGppUmiStreetCanyonPropagationLossModel");
  mmwaveHelperGroup1->SetChannelConditionModelType("ns3::ThreeGppUmiStreetCanyonChannelConditionModel");
 
// Create a second helper with a more lossy model
  Ptr<MmWaveHelper> mmwaveHelperGroup2 = CreateObject<MmWaveHelper>();
// Using UMa model which is more lossy than UMi due to larger distances and more blockage
  mmwaveHelperGroup2->SetPathlossModelType("ns3::ThreeGppUmaPropagationLossModel");
  mmwaveHelperGroup2->SetChannelConditionModelType("ns3::ThreeGppUmaChannelConditionModel");

// Both helpers share same EPC
 // Ptr<MmWavePointToPointEpcHelper> epcHelperGroup1 = CreateObject<MmWavePointToPointEpcHelper>();
  Ptr<MmWavePointToPointEpcHelper> epcHelperGroup1 = CreateObject<MmWavePointToPointEpcHelper>();
  Ptr<MmWavePointToPointEpcHelper> epcHelperGroup2 = CreateObject<MmWavePointToPointEpcHelper>();
  mmwaveHelperGroup1->SetEpcHelper(epcHelperGroup1);
  mmwaveHelperGroup2->SetEpcHelper(epcHelperGroup2);
//  mmwaveHelperGroup2->SetAttribute("EnbCellIdStart", UintegerValue(100));
  //
  ...
  Ptr<Node> pgw = epcHelperGroup1->GetPgwNode ();
  ...
  NodeContainer mmWaveEnbNodesGroup1;
  NodeContainer mmWaveEnbNodesGroup2;
  //
  uint8_t nMmWaveEnbNodesGroup1 = nMmWaveEnbNodes / 2;  // First half
  uint8_t nMmWaveEnbNodesGroup2 = nMmWaveEnbNodes - nMmWaveEnbNodesGroup1;  // Second half
  
  //
  NodeContainer lteEnbNodes;
  NodeContainer allEnbNodes;
  //mmWaveEnbNodes.Create (nMmWaveEnbNodes);
  mmWaveEnbNodesGroup1.Create(nMmWaveEnbNodesGroup1);
  mmWaveEnbNodesGroup2.Create(nMmWaveEnbNodesGroup2);
  //
  mmWaveEnbNodes.Add(mmWaveEnbNodesGroup1);
  mmWaveEnbNodes.Add(mmWaveEnbNodesGroup2);
  //
 ...
 // double nConstellation = nMmWaveEnbNodes - 1;
  //
  double angleStep = 2 * M_PI / (nMmWaveEnbNodes - 1);
  int remainingGroup1 = nMmWaveEnbNodesGroup1 - 1;  // Already placed one in center
  int remainingGroup2 = nMmWaveEnbNodesGroup2;
 ...
 // for (int8_t i = 0; i < nConstellation; ++i)
   ...
   // }
  for (int i = 0; i < remainingGroup1; i++) {
...
// Then place Group 2 eNBs in between
  for (int i = 0; i < remainingGroup2; i++) {
      double angle = (i + 0.5) * angleStep;  // Offset by half the angle step
      x = isd * cos(angle);
      y = isd * sin(angle);
      enbPositionAlloc->Add(Vector(centerPosition.x + x, centerPosition.y + y, 3));
  }
 ...
 // NetDeviceContainer mmWaveEnbDevs = mmwaveHelper->InstallEnbDevice (mmWaveEnbNodes);
  // Replace the existing device installation code
  NetDeviceContainer lteEnbDevs = mmwaveHelperGroup1->InstallLteEnbDevice(lteEnbNodes);
  NetDeviceContainer mmWaveEnbDevsGroup1 = mmwaveHelperGroup1->InstallEnbDevice(mmWaveEnbNodesGroup1);
  NetDeviceContainer mmWaveEnbDevsGroup2 = mmwaveHelperGroup2->InstallEnbDevice(mmWaveEnbNodesGroup2);
// After installing the eNB devices
//
...
//  mmWaveEnbDevs.Add(mmWaveEnbDevsGroup2);
  ...
  NetDeviceContainer mcUeDevs = mmwaveHelperGroup1->InstallMcUeDevice(ueNodes);

  // Install the IP stack on the UEs
  internet.Install (ueNodes);
  Ipv4InterfaceContainer ueIpIface;
  ueIpIface = epcHelperGroup1->AssignUeIpv4Address (NetDeviceContainer (mcUeDevs));
  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ...
      ueStaticRouting->SetDefaultRoute (epcHelperGroup1->GetUeDefaultGatewayAddress (), 1);
    }

  // Add X2 interfaces
  //mmwaveHelper->AddX2Interface (lteEnbNodes, mmWaveEnbNodes);
//  mmwaveHelperGroup1->AddX2Interface(lteEnbNodes, mmWaveEnbNodesGroup1);
//  mmwaveHelperGroup1->AddX2Interface(lteEnbNodes, mmWaveEnbNodesGroup2);
//  mmwaveHelperGroup1->AddX2Interface(mmWaveEnbNodesGroup1, mmWaveEnbNodesGroup2);
  // Manual attachment
 // mmwaveHelper->AttachToClosestEnb (mcUeDevs, mmWaveEnbDevs, lteEnbDevs);
  mmwaveHelperGroup1->AttachToClosestEnb(mcUeDevs, mmWaveEnbDevs , lteEnbDevs);
  ...
  if (enableTraces)
    {
      ...
      mmwaveHelperGroup1->EnableTraces();
      mmwaveHelperGroup2->EnableTraces();
    }
