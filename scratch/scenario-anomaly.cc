...
main (int argc, char *argv[])
{
  ...
  //
  //
  uint32_t numNoisyUes = 6;
  for (uint32_t i = 0; i < numNoisyUes; ++i)
  {
      Ptr<NetDevice> ueDev = mcUeDevs.Get (i);
      Ptr<McUeNetDevice> mcUe = ueDev->GetObject<McUeNetDevice> ();
      if (mcUe)
        {
          // Degrade connection by increasing noise figure
          Ptr<MmWaveUePhy> mmWavePhy = mcUe->GetMmWavePhy ();
          mmWavePhy->SetNoiseFigure (10); // Default is typically 5
          NS_LOG_UNCOND ("Increased noise figure for UE " << i);
        }
  }
